...
import { Building2, Copy, CheckCheck, Package, Loader2, AlertCircle } from "lucide-react";
...
function TransferenciaContent() {
  ...
  // Intentar leer de URL primero, luego de localStorage
  const queryTotal = searchParams.get("total");
  const queryOrderId = searchParams.get("order_id");
  
  const [transferData, setTransferData] = useState<{ total: number; order_id: string } | null>(null);
  ...
  useEffect(() => {
    ...
    // Solo limpiar si no se limpió antes
    if (!hasCleared.current) {
      hasCleared.current = true;
      clearCart();
    }

    // Obtener info del monto y orden
    if (queryTotal && queryOrderId) {
      setTransferData({ total: parseFloat(queryTotal), order_id: queryOrderId });
    } else {
      const pendingData = localStorage.getItem("pending_transfer");
      if (pendingData) {
        setTransferData(JSON.parse(pendingData));
        localStorage.removeItem("pending_transfer"); // Limpiar después de leer
      }
    }
    ...
    fetchSettings();
  }, [router, clearCart, queryTotal, queryOrderId]);
  ...
  return (
    <div className="max-w-3xl mx-auto py-10 px-4">
      <div className="flex flex-col items-center mb-10 text-center">
        ...
        <h1 className="text-3xl font-bold text-text-primary mb-3">Instrucciones de Transferencia</h1>
        <p className="text-text-secondary max-w-md">
          Tu pedido fue registrado exitosamente. Completa el pago realizando una transferencia a la siguiente cuenta.
        </p>
      </div>
      ...
      {transferData && (
        <div className="bg-primary/5 border border-primary/20 rounded-xl p-6 text-center mb-8">
          ...
          <p className="text-4xl font-bold text-primary mb-2">${transferData.total.toFixed(2)} MXN</p>
          <p className="text-sm font-medium text-text-primary">
            Número de orden: <span className="font-mono text-primary">{transferData.order_id.substring(0, 8).toUpperCase()}</span>
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-amber-50 rounded-xl border border-amber-200 p-6 flex flex-col justify-center">
          ...
          <ol className="space-y-3 text-sm text-amber-800 font-medium">
            ...
            <li className="flex gap-2">
              <span className="font-bold">3.</span> Envía el comprobante a los datos indicados.
            </li>
            <li className="flex gap-2">
              <span className="font-bold">4.</span> Edna verificará tu pago y confirmará tu orden manualmente.
            </li>
            <li className="flex gap-2">
              <span className="font-bold">5.</span> Recibirás tu pedido en 3-5 días hábiles tras la confirmación.
            </li>
          </ol>
        </div>
      </div>
